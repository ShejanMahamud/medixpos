...
export function addMissingOpeningBalances(db: Database): void {
  ;('Adding missing opening balance ledger entries...')
  ...
  for (const supplier of suppliers) {
    ...
    if (result.count === 0 && supplier.opening_balance && supplier.opening_balance !== 0) {
      ...
      addedCount++
      ;`Added opening balance entry for supplier ${supplier.id}`
    }
  }

  ;`Migration complete: Added ${addedCount} opening balance ledger entries`
}
