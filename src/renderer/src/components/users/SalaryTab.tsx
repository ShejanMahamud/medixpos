...
export default function SalaryTab({ users, currentUser }: SalaryTabProps): React.JSX.Element {
  ...
  const loadSalaries = async (): Promise<void> => {
    try {
      ...
      const typedSalaries = salariesData as unknown as Salary[]
      const typedPayments = paymentsData as unknown as SalaryPayment[]

      setSalaries(typedSalaries)
      setSalaryPayments(typedPayments)

      // Calculate stats
      const totalEmployees = users.length
      const monthlyPayroll = typedSalaries.reduce(
      ...
      const pendingPayments = typedPayments
