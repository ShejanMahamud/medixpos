...
export default function StockAdjustmentModal({
  ...
  // Get settings for conditional field display
  const stockEnableBatchTracking = useSettingsStore((state) => state.stockEnableBatchTracking)
  const stockShowExpiryOnSale = useSettingsStore((state) => state.stockShowExpiryOnSale)
      ...
      <form onSubmit={handleSubmit}>
        <DialogContent sx={{ pointerEvents: 'auto' }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
            ...
            {/* Show batch number field only if batch tracking is enabled */}
            {stockEnableBatchTracking && (
              <TextField
                fullWidth
                label="Batch Number"
                value={formData.batchNumber}
                onChange={(e) => onFormDataChange({ ...formData, batchNumber: e.target.value })}
                helperText="Optional: Track products by batch/lot number"
              />
            )}

            {/* Show expiry date field only if expiry tracking is enabled */}
            {stockShowExpiryOnSale && (
              <TextField
                fullWidth
                label="Expiry Date"
                type="date"
                ...
                helperText="Track product expiration dates for alerts"
              />
            )}
          </Box>
        </DialogContent>
