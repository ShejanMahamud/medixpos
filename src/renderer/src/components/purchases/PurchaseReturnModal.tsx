...
export default function PurchaseReturnModal({
  ...
  const handlePurchaseSelect = async (purchaseId: string): Promise<void> => {
    ...
    if (purchaseId) {
      try {
        const purchaseData = await window.api.purchases.getById(purchaseId)
        const purchase = purchaseData as unknown as {
          items: Array<{
            id: string
            productId: string
            productName: string
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      await window.api.purchaseReturns.create(
        ...
        returnItems as unknown as Record<string, unknown>[]
      )
